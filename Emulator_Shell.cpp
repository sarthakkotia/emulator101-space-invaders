...
void Emulate(State* state){
    ...
    switch(opcode){
        ...
        case 0x81:
            //ADI byte
            //Arithmetic group -> Immediate Form
            int temp = pc+1;
            uint8_t byte = state->memory[temp];
    ...
    cout<<"CY->"<<state->flag.cy<<" P->"<<state->flag.p<<" S->"<<state->flag.s<<" Z->"<<state->flag.z<<endl;
